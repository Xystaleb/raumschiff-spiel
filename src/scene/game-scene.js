...
export default class GameScene extends Scene {
  ...
  async initializeStage () {
    ...
    if (stage.randomSize !== undefined) {
      for (const rnd of stage.randomSize) {
        const size = Math.random() * (this.BLOCK_WIDTH - this.BLOCK_WIDTH / 2) + this.BLOCK_WIDTH / 2
  ...
  async loop () {
    ...
    if (this.finish <= 0) { await this.nextLevel() }
    // Spiel-Loop wiederholen
    if (!this.sceneState.gameOver) { setTimeout(() => { requestAnimationFrame(this.loop.bind(this)) }, 1000 / 150) }
  }
  ...
  checkCollisions (player) {
    for (let i = 0; i < this.gameObjects.length; i++) {
      const current = this.gameObjects[i]
  ...
  async endGame () {
    ...
    const endGame = new EndGameScene(this.view, playerOne, playerTwo, this.singlePlayer) // Übergabe ob single oder multiplayer
    await endGame.build()
    endGame.draw()
  }
}
